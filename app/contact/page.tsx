...
const ContactLink = ({ href, name }: { href: string; name: string }) => {
    ...
    return (
        <a href={href} rel="noopener noreferrer" target="_blank">
            <TextShadow isLink={true} cursor="pointer" fontSize="28px" color={theme.colors.code.function}>
                {name}
            </TextShadow>
        </a>
    )
    ...
    return (
        <>
            <section style={{ height: '100%', display: 'flex' }}>
                <Container maxW={'container.lg'} pt={24} width={['100%', '100%', '100%', 'fit-content']}>
                    <Flex animation={`1s ${animation}`} mb={5} alignItems="baseline" flexDir="column">
                        <TextShadow fontWeight="bold" mb={3} fontFamily="menlo" fontSize="60px" as="h1">
                            👋🏼
                        </TextShadow>
                        <TextShadow fontWeight="bold" fontFamily="menlo" fontSize="56px" as="h2">
                            Say Hello_
                        </TextShadow>
                        <Text opacity={0.7} mb={5} fontWeight="light" fontSize={['16px', '17px', '22px']}>
